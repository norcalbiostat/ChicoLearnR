...
1. Three coins are tossed and the number of heads $X$ is counted. Simulate the pmf. **Store the pmf in the vector `nheads` provided**

```{r yti2a, exercise = TRUE}
coin <- c("h","t")

coin_toss <- replicate(5,{
  results <- sample(coin,3,replace=TRUE)
  sum(results=="h")
})

nheads <- proportions(table(coin_toss))
```
...
grade_this({
  if (length(.result) != length(nheads.soln)) {
    fail("Incorrect space of the random variable. Did you use sufficient replications?")
  }
...
2. Seven balls number 1-7 are in an urn. Two balls are drawn from the urn without replacement. Let $X$ be the sum of the numbers on the balls. Estimate via simulation the pmf of $X$. **Store the pmf in the vector `sum.balls` provided**
...
grade_this({
  if (.result > min(walk.space) & 
      .result < max(walk.space)) {
    pass("Great work!")
  }
  
  if (.result > max(walk.space)) {
...
grade_this({
  if (.result > min(mm.green.ans.space) & 
      .result < max(mm.green.ans.space)) {
    pass("Great work!")
  }
  
  if (.result > max(mm.green.ans.space)) {
    fail("Your answer is a bit high")
  }
  if (.result < min(mm.green.ans.space)) {
    fail("Your answer is a bit low")
...
grade_this({
  if (.result > min(mm.roy.ans.space) & 
      .result < max(mm.roy.ans.space)) {
    pass("Great work!")
  }
  
  if (.result > max(mm.roy.ans.space)) {
    fail("Your answer is a bit high")
  }
  if (.result < min(mm.roy.ans.space)) {
    fail("Your answer is a bit low")
